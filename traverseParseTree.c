...
typedef struct  {
    ...
    int r1_low;
    int r2_low;
    int tag_dim;//2d or 3d ke liye tagg
...
void traverse_parse_tree(/*receive params t, T*/){
    if(t->__=="START"){
       ...
       //build for the "typeExpression" of this declarartion statement
       ...
       * temp=t;    //just being safe, making copy
       ...
       if(temp->=="Primitive"){
           ...
           * varlist_pointer;     //moving into single/multiple_primitive node
           if(SINGLE){
           ...
           * varlist_pointer; 
           ...
           temp=temp->sibling->sibling;//(ignore just after colon, jagged and arrray written)
            ...
            if(l_bound<=u_bound){//chk =
                ...
                temp=temp->firstChild;
                * temp_jag=temp->firstChild;               
                temp_jag=temp_jag->sibling->sibling;//pointing on NUM
                int indx=temp_jag;
                //chk index in l_bound,u_bound and particularly increasing
           ...
           exp_table_record.record.j_arr_record=temp->child->value(int, real or bool);
